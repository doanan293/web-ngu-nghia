# Báo cáo bài tập: Xây dựng mô hình ngữ nghĩa cho luật doanh nghiệp Việt Nam

## 1. Yêu cầu bài tập

Báo cáo thực hiện các yêu cầu mô hình hóa một phần luật doanh nghiệp Việt Nam sử dụng các công nghệ web ngữ nghĩa (RDF, RDFS, OWL). Cụ thể bao gồm:
...
Mô hình sử dụng không gian tên riêng của bài toán luật doanh nghiệp Việt Nam. Không gian tên này được dùng để định danh các lớp, thuộc tính và đối tượng trong mô hình.
...
- Lớp công ty cổ phần là lớp con của lớp công ty.
- Lớp công ty TNHH một thành viên là lớp con của lớp công ty.
- Lớp công ty hợp danh là lớp con của lớp công ty.
- Lớp cổ đông là lớp con của lớp thành viên.
- Lớp thành viên hợp danh là lớp con của lớp thành viên.
...
### 2.4. Bộ ba minh họa cho từng thuộc tính (sau khi suy luận "subPropertyOf")

Khi thực hiện truy vấn SPARQL đối với thuộc tính cha "có thành viên", hệ thống tự động suy luận và trả về các bộ ba dữ kiện minh họa sau:
- Vinamilk - có thành viên - Tổng công ty SCIC (suy diễn từ thuộc tính con "có cổ đông").
- Hanoi Metro - có thành viên - UBND TP. Hà Nội (suy diễn từ thuộc tính con "có chủ sở hữu").
- Đấu giá Lạc Việt - có thành viên - Đỗ Thị Hồng Hải (suy diễn từ thuộc tính con "có thành viên hợp danh").
...
- Tệp định nghĩa khái niệm (schema): "luat-doanh-nghiep-schema.ttl" chứa khai báo các lớp, thuộc tính và ràng buộc logic OWL.
- Tệp dữ kiện cá thể (data): "luat-doanh-nghiep-data.ttl" chứa thông tin về các cá thể và các bộ ba dữ kiện ban đầu.
